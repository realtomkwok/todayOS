...
export const Timeline = () => {
  ...
  const calculateHeights = () => {
    ...
    const totalHeight = newHourHeights.reduce((sum, height) => sum + height, 0) + 24 * HOUR_LABEL_HEIGHT; // Add height for all hour labels
  ...
  const getHeightBeforeHour = useCallback(
    (hour: number) => {
      return (dimensions.hourHeights.slice(0, hour).reduce((sum: number, height: number) => sum + height, 0) +
  ...
  const getTimeFromScrollPosition = (scrollTop: number) => {
    const adjustedPosition = Math.max(0, scrollTop + dimensions.indicatorOffset - dimensions.paddingTop);
    ...
    for (let hour = 0; hour < 24; hour++) {
      const hourHeight = (dimensions.hourHeights[hour] + HOUR_LABEL_HEIGHT) as number;
    ...
    const minutesInHour = Math.floor((remainingPixels / dimensions.hourHeights[targetHour]) * 60);
  ...
  const scrollToTime = useCallback(
    (time: Date) => {
      ...
      const targetScrollTop = position + dimensions.paddingTop - dimensions.indicatorOffset;
  ...
  const tomorrowEvents = sampleEvents.filter((event) => event.startTime.getDate() === tomorrow.getDate());
  ...
  return (
    <div className="w-full h-screen bg flex flex-col overflow-hidden fixed inset-0">
      ...
      <div ref={containerRef} className="relative flex-1 min-h-0 overflow-hidden">
        ...
        {/* Now */}
        <Now offsetFromTop={OFFSET_FROM_TOP} isInteracting={isInteracting} isTimelineLocked={isTimelineLocked} />
        ...
        <motion.div
          ...
          initial="initial"
          animate={!isInteracting.current && !isTimelineLocked ? "initial" : "visible"}
        ...
        >
          ...
          >
            <div className="flex flex-row items-center gap-2">
              ...
              <div className="relative flex flex-col items-left ">
                <span className="w-20 text-md-on-surface text-xl font-display font-semibold tracking-tight">Today</span>
          ...
          <div
            ...
            style={{
              height: `${dimensions.totalHeight + dimensions.paddingTop + dimensions.paddingBottom}px`,
            ...
            <div className="absolute top-0 w-full h-full" style={{ paddingTop: dimensions.paddingTop }}>
              <AnimatePresence>
                {sampleEvents.map((event: ITimelineEvent) => {
                  ...
                  const durationInPixels = durationInMinutes * dimensions.minuteHeight;
                  ...
                  if (event.endTime) {
                    eventHeight = durationInPixels + HOUR_LABEL_HEIGHT * durationInHours;
                  }
